...
const PaymentPage = () => {
  const [isSuccessDialogOpen, setIsSuccessDialogOpen] = useState(false); // 성공 시 모달
  const [isErrorDialogOpen, setIsErrorDialogOpen] = useState(false); // 실패 시 모달
  ...
  const { mutateAsync, isPending, isError } = usePaymentCafeteriaBarcode();
  ...
  const handleBarcodeScan = async (barcode: string) => {
    ...
    try {
      ...
      if (errorMessage) {
        ...
        appendLatestErrorLog(barcode, errorMessage);
      }
    } finally {
      setTimeout(() => setIsSuccessDialogOpen(false), 2000); // 2초
    }
  };

  useBarcodeScanner(handleBarcodeScan);

  console.log(selectedMenu, '##### status');
